...
- **国内外双源加速**：优先直连 GitHub 官方源；如遇国内 VPS 网络超时，自动平滑切换至镜像加速节点下载。
- **Systemd 服务托管**：自动配置、注册并启用 `frps.service` 系统守护进程，开机自启并支持崩溃重启。
...
sudo frps-admin uninstall   # 卸载 FRPS
```
...
- **Dual Source Acceleration**: Prioritizes direct GitHub download; automatically falls back to mirror acceleration if connection times out.
- **Systemd Service Integration**: Automatically creates, registers, and enables `frps.service` for auto-start on boot and failure recovery.
- **Secure Default Configuration**: Generates `frps.toml` with a secure 16-character random authentication token.
- **Interactive Management Console**: Clean numerical menu for status checks, start, stop, restart, log viewing, and uninstallation.
- **Global Management Shortcut**: Automatically registers the `frps-admin` command for convenient system-wide management.
- **Zero Emoji Style**: Adheres to clean UNIX terminal formatting with zero emojis.
...
Once installed, manage the FRPS service from any terminal location by running:
...
You can also pass arguments directly to `frps-admin`:

```bash
sudo frps-admin start       # Start service
sudo frps-admin stop        # Stop service
sudo frps-admin restart     # Restart service
sudo frps-admin status      # Check service status
sudo frps-admin logs        # Stream real-time logs
sudo frps-admin config      # Display config content
...
By default, FRPS listens on TCP port `7000`. Ensure that inbound traffic on this port is allowed:
